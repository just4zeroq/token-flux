...
// Claude 协议走 Anthropic 兼容端点，OpenAI 走 /v1/text/chatcompletion_v2，图片走 /v1/image/generation。
func (a *Adaptor) GetRequestURL(info *common.RelayInfo) (string, error) {
	...
	default:
		return "", fmt.Errorf("minimax does not support relay mode: %d", info.RelayMode)
	}
}
...
// Claude 入站直接透传到 Anthropic 兼容端点，图片生成做 MiniMax 格式转换，其他格式先转为 OpenAI 再做模型映射。
func (a *Adaptor) ConvertRequest(ctx context.Context, info *common.RelayInfo, requestBody []byte) (io.Reader, error) {
	// 图片生成：OpenAI 格式 → MiniMax 格式
...
// DoResponse 处理上游响应。
// 图片生成走 MiniMax → OpenAI 转换；Claude 入站委托 claude.Adaptor；其他委托 openai.Adaptor。
