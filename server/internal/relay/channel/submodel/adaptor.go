...
// Adaptor Submodel 供应商适配器。
// 仅支持 Chat Completions 端点，其他端点返回错误。
type Adaptor struct {
...
// GetRequestURL 构建上游请求 URL。Submodel 只支持 chat completions。
...
// DoResponse 处理上游响应。Submodel 返回格式与 OpenAI 一致，委托 OpenAI 适配器处理。
