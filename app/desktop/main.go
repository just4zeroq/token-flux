...
// NodeService is bound to the React frontend. All methods are exposed via Wails Bind.
...
func (n *NodeService) Status() map[string]any {
	return map[string]any{
		...
		"models_count":  len(n.router.Models()),
		"keys_count":    n.countKeys(),
		"online":        n.tunnel != nil,
	}
}

func (n *NodeService) Start() string   { return "already running" }
...
func (n *NodeService) GetUsageLogs(limit int) []map[string]any {
	...
	for rows.Next() {
		...
		out = append(out, map[string]any{
			...
			"cost": cost, "latency_ms": lat, "success": success == 1,
			"created_at": created,
		})
	}
	return out
