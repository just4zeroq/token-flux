...
// Convert translates a request body from one format to another.
// Uses direct paths (openai↔claude, openai↔gemini) when available.
// Falls back to Normalize(from→openai) → Denormalize(openai→to) via hub.
func Convert(body []byte, from, to Format) ([]byte, error) {
	if from == to {
		return body, nil
	}
	// Direct paths.
	if from == FormatOpenAI && to == FormatClaude {
		return OpenAIToClaude(body)
	}
	if from == FormatOpenAI && to == FormatGemini {
		return OpenAIToGemini(body)
	}
	// Hub fallback: Normalize(from→openai) → Denormalize(openai→to).
	normalized, err := Normalize(body, from)
	if err != nil {
		return nil, fmt.Errorf("convert %s→%s: normalize: %w", from, to, err)
	}
	dn, err := Denormalize(normalized, to, false)
	if err != nil {
		return nil, fmt.Errorf("convert %s→%s: denormalize: %w", from, to, err)
	}
	return dn.ConvertBody(normalized)
}

// DetectFormatFromPath determines format from the request path.
func DetectFormatFromPath(path string) (Format, bool) {
	...
	return FormatOpenAI, false
}

// Detect determines format from path first, falls back to body detection.
