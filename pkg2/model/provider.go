// Package model defines shared data types used by both server relay and local node.
// Types map to database tables (PostgreSQL for server, SQLite for node).
package model

// ProviderType maps to llm_channels.provider_type / chn_channels.type.
// Values are compatible with server/internal/relay/constant.
...
const (
	...
	ProtocolAli       ProtocolType = "ali"
	ProtocolBaidu     ProtocolType = "baidu"
	ProtocolZhipu     ProtocolType = "zhipu"
	ProtocolVolcengine ProtocolType = "volcengine"
	ProtocolAWS       ProtocolType = "aws"
)
...
// RelayMode identifies the API mode (chat, completions, embeddings, etc.).
type RelayMode int
...
const (
	...
	RelayModeImagesGenerations RelayMode = 4
	RelayModeAudioSpeech       RelayMode = 5
	RelayModeAudioTranscription RelayMode = 6
	RelayModeClaudeMessages    RelayMode = 7
	RelayModeGeminiChat        RelayMode = 8
	RelayModeResponses         RelayMode = 9
)
